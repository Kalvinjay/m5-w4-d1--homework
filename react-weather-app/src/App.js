...
function App() {
  //State
  const [apiData, setApiData] = useStage({});
  const [getState, setGetState] = useState('Winnipeg, CAN');
  const [state, setState] = useState('Winnipeg, CAN');

  //API KEY AND URL
  const apiKey = process.env.REACT_APP_API_KEY;
  const appUrl = `https://api.openweathermap.org/data/2.5/weather?q=${state}&appid=${apiKey}`;
              ...
              <div className="row mt-4">
                <div className="col-md-6">
                  <p>
                    ...
                    /> {' '}
                    <strong>
                      {kelvinToFarenheit(apiData.main.temp_min)}&default; F
                    </strong>
                  </p>
                  <p>
                    <FontAwesomeIcon
